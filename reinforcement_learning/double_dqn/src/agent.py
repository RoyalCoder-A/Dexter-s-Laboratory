...
import numpy as np
import torch
from reinforcement_learning.double_dqn.src.memory import Memory
from reinforcement_learning.double_dqn.src.network import Network
...
class Agent:
    ...
    def learn(self):
        if self.step >= self.batch_size:
            ...
            self.q_main.eval()
            with torch.inference_mode():
                next_target_q_values = self.q_target(states__t)
                next_main_q_values = self.q_main(states__t)
                next_actions = torch.argmax(next_main_q_values, dim=1)
