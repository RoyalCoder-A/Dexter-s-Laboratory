...
import numpy as np
import torch
from reinforcement_learning.dueling_ddqn.src.memory import Memory
from reinforcement_learning.dueling_ddqn.src.network import Network
...
class Agent:
    def __init__(
        ...
        self.optimizer = torch.optim.RMSprop(self.q_main.parameters(), lr=learning_rate)
        self.eps = eps
    ...
    def get_action(self, state: np.ndarray) -> int:
        ...
        with torch.inference_mode():
            ...
            _, advantages = self.q_main(
                states_tensor.view(1, *self.states_dim).to(self.device)
            )
        return int(np.argmax(advantages.cpu().numpy()[0]))
    ...
    def learn(self):
        if self.step >= self.batch_size:
            ...
            self.q_main.eval()
            with torch.inference_mode():
                next_target_state_values, next_target_advantages = self.q_target(
                    states__t
                )
                next_target_q_values = next_target_state_values + (
                    next_target_advantages
                    - torch.mean(next_target_advantages, dim=1, keepdim=True)
                )
                _, next_main_advantages = self.q_main(states__t)
                next_actions = torch.argmax(next_main_advantages, dim=1)
                next_q_values = next_target_q_values[indices, next_actions]
                next_q_values[terminals_t] = 0.0
            self.q_main.train()
            state_values, advantages = self.q_main(states_t)
            q_values = (
                state_values
