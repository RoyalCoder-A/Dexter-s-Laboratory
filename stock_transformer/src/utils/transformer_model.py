...
from stock_transformer.src.utils.dataset import FEATURES_COUNT, TARGETS_COUNT
...
class TransformerModel(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.encoder_pre_layer = PreLayer(FEATURES_COUNT, d_model)
        self.decoder_pre_layer = PreLayer(TARGETS_COUNT, d_model)
        self.encoder = Encoder(n, d_model, dff, n_heads, p_dropout)
        self.decoder = Decoder(n, d_model, dff, n_heads, p_dropout)
        self.head = torch.nn.Linear(d_model, TARGETS_COUNT)
    ...
    def forward(
        ...
        decoder_input = self.decoder_pre_layer(decoder_input)
