...
class Agent:
    ...
    def step(self) -> None:
        ...
        actions = torch.from_numpy(self.memory.actions).long().to(self.device)
        ...
        self.opt.zero_grad()
        with torch.no_grad():
            next_probs = self.q_target(states_)
            target_probs = get_target_probs(
                next_probs, actions, rewards, terminals, self.limits, self.discount
            )
